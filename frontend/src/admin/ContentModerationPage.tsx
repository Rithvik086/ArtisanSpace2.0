...
export default function ContentModerationPage(): React.ReactElement {
  ...
  const normalize = (items: any[]) => items.map((p) => ({
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const data = res.data;
      const items = Array.isArray(data?.products ? data.products : data) ? (data.products ?? data) : [];
      setProducts(normalize(items as any[]));
    } catch (e) {
      console.error('Failed to load moderation products', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getTabContent = (): React.ReactElement => {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {productsToShow.map(p => <ModerationProductCard key={p.id} product={p} onModerated={(id, status) => setProducts(prev => prev.map(it => it.id === id ? { ...it, status } : it))} />)}
      </div>
    );
  };
