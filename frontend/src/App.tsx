  ...
  const features = [
    {
      ...
      description: 'Upload high-quality photos and 3D models of your work to a beautiful, customizable storefront.',
    },
    {
      icon: <ShoppingBag size={32} className="text-amber-800" />,
      title: 'Seamless E-Commerce',
      description: 'Manage inventory, process payments, and handle shipping all from one simple, integrated dashboard.',
    },
    {
      icon: <Users size={32} className="text-amber-800" />,
      title: 'Build Community',
      description: 'Connect with other artisans, share techniques, and engage directly with customers who love your work.',
    },
  ];
    ...
    <section id="features" className="py-20 md:py-32 bg-amber-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto">
          <h2 className="text-base font-semibold text-amber-800 tracking-wide uppercase">Features</h2>
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-10">
          {features.map((feature) => (
            <div key={feature.title} className="bg-white p-8 rounded-xl shadow-lg hover:shadow-2xl transition-shadow duration-300 transform hover:-translate-y-1">
              <div className="flex items-center justify-center h-12 w-12 rounded-full bg-amber-100">
                {feature.icon}
              </div>
              <h3 className="mt-6 text-2xl font-bold text-amber-950">{feature.title}</h3>
              <p className="mt-4 text-base text-amber-900">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
...
function TestimonialsSection() {
  const testimonials = [
    {
      quote: "ArtisanSpace transformed my hobby into a business. The 3D viewer is a game-changer for my pottery!",
      name: 'Sarah Chen',
      title: 'Potter, Chen\'s Ceramics',
      avatar: 'https://placehold.co/100x100/d4b996/5c4033?text=SC'
    },
    {
      quote: "Finally, a platform that understands artisans. The community features are fantastic for collaborating.",
      name: 'David Lee',
      title: 'Jewelry Maker, Lee Metals',
      avatar: 'https://placehold.co/100x100/d4b996/5c4033?text=DL'
    },
    {
      quote: "I was hesitant about selling online, but ArtisanSpace made it incredibly simple. My sales have doubled!",
      name: 'Maria Gonzalez',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial) => (
            <div key={testimonial.name} className="flex flex-col bg-white rounded-xl shadow-lg overflow-hidden">
              ...
              <div className="bg-amber-100 p-6 flex items-center space-x-4">
                ...
                <div>
                  <p className="text-base font-bold text-amber-950">{testimonial.name}</p>
                  <p className="text-sm font-medium text-amber-800">{testimonial.title}</p>
                </div>
              </div>
            </div>
          ))}
    ...
    <section className="bg-linear-to-r from-[#5c4033] to-[#4a3227] py-20 md:py-32">
      <div className="max-w-4xl mx-auto text-center px-4 sm:px-6 lg:px-8">
        ...
        <p className="mt-6 text-xl text-amber-100 max-w-2xl mx-auto">
          Ready to turn your passion into your profession? Open your shop on ArtisanSpace today and start selling.
        </p>
        <div className="mt-10">
          <button className='px-12 py-5 bg-white text-amber-950 rounded-lg font-semibold text-xl hover:bg-amber-50 transition-colors duration-300 shadow-2xl hover:shadow-xl transform hover:-translate-y-1'>
            Sign Up for Free
            <ArrowRight className="inline-block ml-2" size={20} />
          </button>
        </div>
        <p className="mt-6 text-sm text-amber-200">No setup fees. No monthly fees.</p>
      </div>
    </section>
  );
}
      ...
      <div className="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          ...
          {Object.keys(footerLinks).map((category) => (
            <div key={category}>
              <h3 className="text-sm font-semibold text-amber-950 tracking-wider uppercase">{category}</h3>
              <ul className="mt-4 space-y-4">
                {footerLinks[category].map((item) => (
                  <li key={item.name}>
                    <a href={item.href} className="text-base text-amber-900 hover:text-amber-950">
                      {item.name}
                    </a>
                  </li>
                ))}
          ...
          <div className="col-span-2 md:col-span-1">
             <h3 className="text-sm font-semibold text-amber-950 tracking-wider uppercase">Subscribe</h3>
             <p className="mt-4 text-base text-amber-900">Get inspiration and updates from the artisan world.</p>
        ...
        <div className="mt-12 pt-8 border-t border-amber-200 flex flex-col md:flex-row justify-between items-center">
          <p className="text-base text-amber-900">&copy; {new Date().getFullYear()} ArtisanSpace. All rights reserved.</p>
          <div className="flex space-x-6 mt-4 md:mt-0">
            {socialLinks.map((item) => (
              <a key={item.name} href={item.href} className="text-amber-900 hover:text-amber-950">
